...
import { TransactionContext } from '../types/mod.ts';
import { logger, timestampWithMs } from '../utils/mod.ts';
  ...
  public add(span: Span): void {
    ...
    if (span.spanId !== this.transactionSpanId) {
      ...
      span.finish = (endTimestamp?: number) => {
        span.endTimestamp = typeof endTimestamp === 'number'
          ? endTimestamp
          : timestampWithMs();
...
export class IdleTransaction extends Transaction {
  ...
  public finish(endTimestamp: number = timestampWithMs()): string | undefined {
    ...
    if (this.spanRecorder) {
      ...
      true && logger.log('[Tracing] flushing IdleTransaction');
    } else {
      true && logger.log('[Tracing] No active IdleTransaction');
    }
  ...
  public initSpanRecorder(maxlen?: number): void {
    if (!this.spanRecorder) {
      ...
      true && logger.log('Starting heartbeat');
      this._pingHeartbeat();
    }
  ...
  private _pushActivity(spanId: string): void {
    this._cancelIdleTimeout();
    true && logger.log(`[Tracing] pushActivity: ${spanId}`);
  ...
  private _popActivity(spanId: string): void {
    if (this.activities[spanId]) {
      true && logger.log(`[Tracing] popActivity ${spanId}`);
