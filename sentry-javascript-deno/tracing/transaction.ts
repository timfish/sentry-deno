...
export class Transaction extends SpanClass implements TransactionInterface {
  ...
  public setName(
    name: string,
    source: TransactionMetadata['source'] = 'custom',
  ): void {
    this.name = name;
  ...
  public finish(endTimestamp?: number): string | undefined {
    ...
    if (this._trimEnd && finishedSpans.length > 0) {
      this.endTimestamp =
        finishedSpans.reduce((prev: SpanClass, current: SpanClass) => {
  ...
  public getBaggage(): Baggage {
    ...
    // empty and mutable
    const finalBaggage = !existingBaggage || isBaggageMutable(existingBaggage)
